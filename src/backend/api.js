...
import { appwriteConfig, account, databases, storage, avatars } from "./auth";
...
export async function createPost(post) {
  try {
    ...
    const uploadedFile = await uploadFile(post.formData.file[0]);
    ...
    const tags = post.formData.tags?.replace(/ /g, "").split(",") || [];
    ...
    const newPost = await databases.createDocument(
      ...
      {
        creator: post.userId,
        caption: post.formData.caption,
        imageUrl: fileUrl,
        imageID: uploadedFile.$id,
        location: post.formData.location,
        tags: tags,
      }
    );
